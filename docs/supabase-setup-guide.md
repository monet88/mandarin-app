...
Sau khi database xong, bạn tiến hành triển khai (deploy) các Edge Functions:
```bash
...
App sử dụng Edge Functions để gọi OpenRouter, cần cung cấp hai keys sau cho Supabase Cloud:
```bash
...
Sau đó, bạn cần thiết lập `.env` hỗ trợ cho function chạy ngầm để nạp `OPENROUTER_API_KEY`.
